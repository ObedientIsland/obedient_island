class EventPartsController < ApplicationController
  before_action :set_event_part, only: [:show, :edit, :update, :destroy]
  before_action :load_events, only: [:edit, :new]
  ...
  private
    ...
    def event_part_params
      params.require(:event_part).permit(:title, :text, :next_type, :next_id, :image, :event_id)
    end
end
